...
static const uint8_t fonts[96][5] =
    {
...
static const uint8_t progressBarSet[11][6][5] =
    {
        {
            // V1
            //  empty
...
static const uint8_t progressBarSet1[2][20] =
    {
        {0x3E, 0x7F, 0x7F, 0x7F, 0x7F,
         0x7F, 0x7F, 0x7F, 0x7F, 0x7F,
         0x7F, 0x7F, 0x7F, 0x7F, 0x7F,
...
void OLED::begin()
{
    ...
    i2c_program_init(pio, sm, offset, 2, 3);
...
OLED &OLED::operator<<(const std::string& string)
{
    outMode = 0;
    count = 0;
...
void OLED::execute(uint8_t instruction) {
    ...
    buf[1] = instruction;
    pio_i2c_write_blocking(pio, sm, address, buf, 2);
}

void OLED::print(const std::string& string, uint8_t x, uint8_t y)
{
    step = 0;
    charWidth = 0;
    if (IS_SETUP)
...
void OLED::printHighlighted(const std::string& string, uint8_t x, uint8_t y)
{
    step = 0;
    charWidth = 0;
    if (IS_SETUP)
...
void OLED::printAnimated(const std::string& string, uint8_t x, uint8_t y, int delay, bool highlight)
{
    step = 0;
    charWidth = 0;
    if (IS_SETUP)
...
void OLED::convert(const std::string &input)
{
    if (tempString != nullptr)
        delete[] tempString;

    tempString = new char[input.length() + 1];
    std::copy(input.begin(), input.end(), tempString);
    tempString[input.length()] = '\0';
}

char *OLED::convertString(const std::string &input)
{
    if (tempString != nullptr)
        delete[] tempString;
...
void OLED::print_c(const std::string& string, uint8_t x, uint8_t y)
{
    step = 0;
    charWidth = 0;
    if (IS_SETUP)
...
void OLED::clearArea(uint8_t startX, uint8_t endX, uint8_t page)
{
    ...
    for (uint8_t i = 0; i < std::abs(diff); i++)
    {
        sendData(0x00);
    }
}
...
void OLED::getFont(char c, bool highlight, uint8_t y)
{
    if (c == '\n' || WIDTH - charWidth < fontWidth)
    {
        ...
        if (c != '\n')
        {
            ...
            {
                drawFontPixel(highlight ? ~fontSet[c - 32][i] : fontSet[c - 32][i], charWidth, y + 8);
                charWidth++;
            }
    ...
    else
    {
        ...
        {
            charWidth++;
            drawFontPixel(highlight ? ~fontSet[c - 32][i] : fontSet[c - 32][i], charWidth, y);
        }
        charWidth++;
    ...
    if (style < 11)
    {
        for (uint8_t i = 1; i <= 16; i++)
        {
            for (uint8_t j = 0; j < 5; j++)
            {
                ...
                {
                    if (i == 1)
                        sendData(progressBarSet[style - 1][3][j]);
                    else if (i == 16)
                        sendData(progressBarSet[style - 1][4][j]);
                    else if (i == progress && style == 2)
                        sendData(progressBarSet[style - 1][4][j]);
                    else
                        sendData(progressBarSet[style - 1][5][j]);
                }
                else
                {
                    if (i == 1)
                        sendData(progressBarSet[style - 1][0][j]);
                    else if (i == 16)
                        sendData(progressBarSet[style - 1][1][j]);
                    else
                        sendData(progressBarSet[style - 1][2][j]);
                }
            }
        }
    }
    else if (style > 13)
    {
        ...
        for (uint8_t i = 0; i < 13; i++)
        {
            ...
            if (i == progress)
            {
                ...
                {
                    sendData(progressBarSet1[style - 14][j]);
                }
            }
            else
            {
    ...
    else
    {
        if (progress < 15)
        {
            ...
            for (uint8_t i = 0; i <= 16; i++)
            {
                ...
                {
                    if (i == progress && i < 15)
                        sendData(progressBarSet[10][setVal][j]);
                    else if (i == progress + 1 && i < 16 && style == 11)
                        sendData(progressBarSet[10][setVal + 1][j]);
                    else
                        sendData(0x00);
                }
            }
        ...
        for (uint8_t row = 0; row < width / 8; row++)
        {
            for (uint8_t b = 0; b < 8; b++)
            {
                ...
                for (uint8_t col = 0; col < 8; col++)
                    byte |= ((dataSet[((width / 8) * 8 * multiplier) + (col * (width / 8) + row)] & (0x80 >> b)) >> 7 - b) << col;
...
void OLED::setPowerMode(uint8_t mode)
{
    if (mode > 0x00 && mode <= 0x04)
    {
        ...
        case TURBO_MODE:
            ...
            div = (float)clock_get_hz(clk_sys) / (10000000 * 5.0f);
            pio_sm_set_clkdiv(pio, sm, div);
            break;
        case PERFORMANCE_MODE:
            performancePowerMode();
            div = (float)clock_get_hz(clk_sys) / (4000000 * 5.0f);
            pio_sm_set_clkdiv(pio, sm, div);
            break;
        case BALANCED_MODE:
            balancedPowerMode();
            div = (float)clock_get_hz(clk_sys) / (2000000 * 5.0f);
            pio_sm_set_clkdiv(pio, sm, div);
            break;
        case LOW_POWER_MODE:
            lowPowerMode();
            div = (float)clock_get_hz(clk_sys) / (1000000 * 5.0f);
            pio_sm_set_clkdiv(pio, sm, div);
            break;
        }
    }
...
void OLED::bitBarPlot(uint8_t x, uint8_t y, uint8_t width, uint8_t height, int *data, uint8_t size, int maxVal, int median)
{
    ...
    {
        int barHeight = scaleFactor * std::abs(data[i]);
...
void OLED::scatterPlot(uint8_t x, uint8_t y, uint8_t width, uint8_t height, int *data, uint8_t size, int maxVal, int median)
{
    ...
    int axis = y + height / 2;
    int medianLineY = axis - std::round(scaleFactor * median);
    line(x, medianLineY, x + width, medianLineY, 2);

    uint8_t step = std::max(1, width / size);

    for (uint8_t i = 0; i < size && x + i * step < x + width; i++)
    {
        uint8_t px = x + i * step;
        int py = axis - std::round(scaleFactor * (data[i] + (median != 0 && (median * data[i]) < median) ? 1 : -1));
        if (py >= y && py < y + height)
            drawPixel(px, py);
    }
}
